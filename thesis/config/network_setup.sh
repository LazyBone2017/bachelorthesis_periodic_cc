...
function setup() {
    ...
    ip netns exec $NS1 tc qdisc add dev $VETH1 root netem delay $DELAY loss $LOSS rate $RATE limit 400
    ip netns exec $NS2 tc qdisc add dev $VETH2 root netem delay $DELAY loss $LOSS rate $RATE limit 400
